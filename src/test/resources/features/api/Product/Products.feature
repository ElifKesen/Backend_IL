Feature: As an administrator, I want to access products via an API connection.
  ...
  (student or teacher) authorization credentials, the response status code is 401, the remark in the response body is
  ...
  authorization credentials, the response status code is 401, and the message in the response body is "Unauthenticated."
    ...
    * The api user sends a "GET" request, saves the returned response, and verifies that the status code is '401' with the reason phrase Unauthorized.
      #API kullanıcısı bir "GET" isteği gönderir, döndürülen yanıtı kaydeder ve durum kodunun '401' olduğunu ve neden ifadesinin Yetkisiz olduğunu doğrular.
